...
ddsg <- readRDS(snakemake@input[[1]])
ddstx <- readRDS(snakemake@input[[2]])
obj_list <- list('Tx' = ddstx, 'Gene' = ddsg)

for (i in 1:length(obj_list)) {
    dds <- obj_list[[i]]
    type <- names(obj_list)[i]
    if (type == 'Tx') {
        print("Running DE analysis for tx-level features")
        prefix = 'tx'
    } else {
        print("Running DE analysis for gene-level features")
        prefix = 'gene'
    }
    print(dds)
    ...
    if (type == 'Tx') {
        mle_res$gene_id <- as.character(rowData(dds)$gene_id)
        map_res$gene_id <- as.character(rowData(dds)$gene_id)
    }
    ...
    if (type == 'Tx') {
     mle_df <- mle_res %>%
      data.frame() %>%
      tibble::rownames_to_column(var = "enstx") %>%
      as_tibble() %>%
      dplyr::select(symbol, enstx, gene_id, everything()) %>%
      dplyr::arrange(padj)
    ...
    readr::write_csv(mle_df, snakemake@output[[paste0(prefix, '_mleres')]])
    readr::write_csv(map_df, snakemake@output[[paste0(prefix, '_mapres')]])
    ggsave(snakemake@output[[paste0(prefix, '_mlema')]], plot = mleplot, width = 10, height = 7)
    ggsave(snakemake@output[[paste0(prefix, '_mapma')]], plot = mapplot, width = 10, height = 7)
}
